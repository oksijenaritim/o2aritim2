...
if st.session_state.user_role:
    ...
    with tab3:
        ...
        for m in st.session_state.data:
            with st.expander(f"🛠 {m['ad']} - {m['cihaz']}"):
                col_a, col_b = st.columns(2)
                # Navigasyon Linki
                nav_url = f"https://google.com{urllib.parse.quote(m['adres'])}"
                col_a.link_button("📍 Navigasyonu Başlat", nav_url, use_container_width=True)
                # WhatsApp Linki
